...
resource "aws_iam_role" "kubernetes_ack" {
  count              = var.enabled ? length(var.helm_services) : 0
  name               = "${var.cluster_name}-ack-${var.helm_services[count.index].name}-ack"
...
resource "aws_iam_role_policy_attachment" "kubernetes_ack" {
  ...
  policy_arn = coalesce(var.helm_services[count.index].policy_arn, "arn:${var.aws_partition}:iam::aws:policy/Amazon${var.helm_services[count.index].name}FullAccess")
}
